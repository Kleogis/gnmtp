...
tf.keras.layers.RNN(tf.keras.layers.GRUCell(4))
        ...
        decoded_size = [self.unit_size for i in range(self.n_layer)]
        encoded_size = [self.unit_size for i in range(self.n_layer)]
        
        with tf.variable_scope('decode'):
            decode_cell=[]
            for i in decoded_size[:]:
                decode_cell.append(tf.keras.layers.RNN(tf.keras.layers.GRUCell(i)))
            self.decoder = tf.nn.rnn_cell.MultiRNNCell(decode_cell)
        
        with tf.variable_scope('encode'):
            encode_cell=[]
            for i in encoded_size[:]:
                encode_cell.append(tf.keras.layers.Conv1D(i))s
        ...
        self.weights['softmax'] = tf.get_variable("softmaxw", initializer=tf.random_uniform(shape=[decoded_size[-1], self.vocab_size], minval = -0.1, maxval = 0.1))       
    ...
    def encode(self): 
        ...
        _, state = tf.nn.dynamic(self.encoder, inp, dtype=tf.float32, scope = 'encode', sequence_length = self.L)
        c,h = state[-1]
    ...
    def decode(self, Z):
        ...
        Y, self.output_decoded_state = tf.nn.dynamic(self.decoder, inputs, dtype=tf.float32, scope = 'decode', sequence_length = self.L, initial_state=self.initial_decoded_state)
    ...
    def sample(self, latent_vector, c, start, seq_length):
